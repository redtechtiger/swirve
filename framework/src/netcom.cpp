...
// FIX THIS SHIT
// THIS WILL MAKE THE DUMPSTER FIRE NOT CRASH (HOPEFULLY)
// me, 5 hrs later: it didn't work
#include <csignal>
#include <fcntl.h>
#include <algorithm>
...
void NetworkCommunicator::serverLoop(bool* shouldStop) { // Function called by fork
    ...
    while(!*shouldStop) {
        ...
        if(iConnection.sockfd<0&&errno!=EAGAIN&&errno!=EWOULDBLOCK) { // Erro
...
int NetworkCommunicator::ReadIncomingConnections(std::vector<Connection> &connections_out, map<int, bool>* auth) {
    std::vector<size_t> connections_closing = std::vector<size_t>();
    for(size_t index=0;index<connections.size();index++) {
        ...
        if(_bytesRead<0) { // Read failed or nothing to read
            if(errno!=EAGAIN||errno!=EWOULDBLOCK) { // Read failed
                Logger l;
                if(errno==9) {
                    l.warn("Netcom","Read failed (BAD FILE DESCRIPTOR), killing connection!");
                    CloseConnection(connection->sockfd);
                    connections.erase(std::find(connections.begin(), connections.end(), *connection));
                    continue;
                }
                l.warn((string)"Netcom","Read failed with errno "+to_string(errno)+(string)", " + strerror(errno) + ", on connection FD "+to_string(connection->sockfd));
                return -1;
            } else {
                connection->buffer = {0};
                continue;
            ...
            connections_closing.push_back(index);
    ...
    for (const auto &conn : connections_closing) {
...
int NetworkCommunicator::WriteIncomingConnection(const int sockfd, const std::string buffer) {
    ...
    for(auto &connection : connections) {
        if(connection.sockfd==sockfd) { // Found connection to write to
            int ret;
            try {
                ret = send(connection.sockfd, buffer.c_str(), buffer.size(),MSG_NOSIGNAL);
            } catch(...) {
                Logger l;
                l.warn((string)"Netcom","Write failed with unknown exception (broken pipe?)");
                return -1;
            }
            if(ret<0) {
                Logger l;
                if(errno==9) {
                    l.warn("Netcom","Write failed (BAD FILE DESCRIPTOR), killing connection!");
                    CloseConnection(connection.sockfd);
                    connections.erase(std::find(connections.begin(), connections.end(), connection));
                }
