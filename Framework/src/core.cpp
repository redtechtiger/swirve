...
void showLogo() {
    ...
    cout << "---===--- Swirve Framework v1.1-DEV" << buf << " ---===---\n";
...
void shell(NetworkCommunicator* networkCommunicator, unique_ptr<ActiveParser>* activeParser, map<unsigned long, shared_ptr<ServerModule>>* globalModules) { // TODO: FINISH SHELL
    cout << endl;
    string buffer;
    int assignedModule;
    while(true) {
        ...
        if(buffer=="help") {
            ...
            cout << "help                       Lists all avalible commands.\n";
            cout << "id set                     Set the assigned module ID.\n";
            cout << "id get                     Gets the assigned module ID.\n";
            cout << "id poll                    Gets all modules loaded in memory.\n";
            cout << "power get                  Gets the power state.\n";
            cout << "power start                Starts the server.\n";
            cout << "power stop                 Stops the server.\n";
            cout << "power restart              Restarts the server.\n";
            cout << "power kill                 Kills the server.\n";
            cout << "log set                    Sends input to server.\n";
            cout << "log get                    Gets output of server.\n";
            cout << "frameworkapi stop          Stops Swirve Framework.\n";
            cout << "frameworkapi killall       Kills all connections.\n";
            cout << "frameworkapi stoplistener  Blocks any new incoming connections to the API.\n";
            cout << "frameworkapi startlistener Resumes listener.\n";
            cout << "frameworkapi stopparser    Stops the parsing of new API calls.\n";
            cout << "frameworkapi startparser   Resumes the parser.\n";
            continue;
        }
        if(buffer=="id set") {
            int id;
        ...
        if(buffer=="id get") {
            cout << assignedModule << endl;
            continue;
        }
        if(buffer=="id poll") {
            for(auto moduleIterator : *globalModules) {
                cout << moduleIterator.second->ID << "\t";
            }
            cout << "\n";
        }
        if(buffer=="power get") {
            cout << (*globalModules)[assignedModule]->State() << endl;
            continue;
        }
        if(buffer=="power start") {
            if(0>(*globalModules)[assignedModule]->Start()) {
                cout << "Illegal operation" << endl;
            }
            continue;
        }
        if(buffer=="power stop") {
            if(0>(*globalModules)[assignedModule]->Stop()) {
                cout << "Illegal operation" << endl;
            }
            continue;
        }
        if(buffer=="power restart") {
            if(0>(*globalModules)[assignedModule]->Restart()) {
                cout << "Illegal operation" << endl;
            }
            continue;
        }
        ...
        if(buffer=="power kill") {
            if(0>(*globalModules)[assignedModule]->Kill()) {
                cout << "Illegal operation" << endl;
            }
            continue;
        }
        if(buffer=="frameworkapi stop") {
            break;
        }
    }
}
